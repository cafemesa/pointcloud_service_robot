					 ...
					 frame_id("map")
{
	...
	PoseSub = ns_.subscribe(pose_topic_sub, 50, &semanticMappingPeople::poseAMCLCallback, this);
...
/** Robot Pose Callback */
void semanticMappingPeople::poseAMCLCallback(const nav_msgs::Odometry::ConstPtr& msg)
{	
	pose_x = (double)initial_robotpose_x+msg->pose.pose.position.x;
	pose_y = (double)initial_robotpose_y+msg->pose.pose.position.y;
	float siny_cosp = 2 * ((double)msg->pose.pose.orientation.w * msg->pose.pose.orientation.z);
	float cosy_cosp = 1 - (2 * ((double)msg->pose.pose.orientation.z * msg->pose.pose.orientation.z));
	initial_angle2 = (double)initial_robotpose_a+atan2(siny_cosp, cosy_cosp);
